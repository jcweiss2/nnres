...
trX, trY, teX, teY = trX.to(cudaFlag), trY.to(cudaFlag), teX.to(cudaFlag), teY.to(cudaFlag)
trYwide = trY
...
model = build_model(n_features, n_classes).to(cudaFlag)
...
re_model = Random_Effects(trX.size()[1], n_classes, noise_size, hidden_size, output_activation).to(cudaFlag)
...
z = torch.FloatTensor(torch.Size((batch_size,noise_size,noise_replicates))).to(cudaFlag)
...
plt.hist(re_model.decoder(teX[:1000],
                          torch.FloatTensor(1000,z.size()[1]).to(cudaFlag).normal_()[:1000]).
